    ...
    def apply_to(self, dataset, split='train'):
        ...
        for i, g in enumerate(dataset):
            if saved_pos_enc is None:
                sc_g = Data(x=g.sc_x, edge_index=g.sc_edge_index,
                            num_nodes=g.num_nodes, edge_attr=g.edge_attr)
                fc_g = Data(x=g.fc_x, edge_index=g.fc_edge_index,
                            num_nodes=g.num_nodes, edge_attr=g.edge_attr)
...
class LapEncoding(PositionEncoding):
    ...
    def compute_pe(self, graph):
        ...
        L = to_scipy_sparse_matrix(
            edge_index, edge_attr, num_nodes=graph.num_nodes).tocsc()
