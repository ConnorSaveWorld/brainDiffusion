...
class GraphDataset(object):
    ...
    def compute_degree(self):
        ...
        for g in self.dataset:
            sc_deg = 1. / \
                torch.sqrt(1. + utils.degree(g.sc_edge_index[0], g.num_nodes))
            self.sc_degree_list.append(sc_deg)

            fc_deg = 1. / \
                torch.sqrt(1. + utils.degree(g.fc_edge_index[0], g.num_nodes))
    ...
    def collate_fn(self):
        def collate(batch):
            ...
            if self.n_tags is None:
                padded_x_sc = torch.zeros(
                    (len(batch), max_len_sc, self.n_features_sc))
                padded_x_fc = torch.zeros(
                    (len(batch), max_len_fc, self.n_features_fc))
            else:
                # discrete node attributes
                padded_x_sc = torch.zeros(
                    (len(batch), max_len_sc, self.n_tags))
                padded_x_fc = torch.zeros(
                    (len(batch), max_len_fc, self.n_tags))

            mask_sc = torch.zeros((len(batch), max_len_sc), dtype=bool)
            mask_fc = torch.zeros((len(batch), max_len_fc), dtype=bool)
            adjs_sc = torch.zeros(
                (len(batch), max_len_sc, max_len_sc), dtype=torch.float32)
            adjs_fc = torch.zeros(
                (len(batch), max_len_fc, max_len_fc), dtype=torch.float32)
            ...
            fc_pos_enc = None
            use_sc_pe = hasattr(
                batch[0], 'sc_pe') and batch[0].sc_pe is not None
            if use_sc_pe:
                if not batch[0].sc_pe.is_sparse:
                    sc_pos_enc = torch.zeros(
                        (len(batch), max_len_sc, max_len_sc))
                else:
                    print("Not implemented yet!")
            use_fc_pe = hasattr(
                batch[0], 'fc_pe') and batch[0].fc_pe is not None
            if use_fc_pe:
                if not batch[0].fc_pe.is_sparse:
                    fc_pos_enc = torch.zeros(
                        (len(batch), max_len_fc, max_len_fc))
                else:
                    print("Not implemented yet!")
            ...
            fc_lap_pos_enc = None
            use_lap_pe = hasattr(
                batch[0], 'sc_lap_pe') and batch[0].sc_lap_pe is not None
            if use_lap_pe:
                sc_lap_pe_dim = batch[0].sc_lap_pe.shape[-1]
                sc_lap_pos_enc = torch.zeros(
                    (len(batch), max_len_sc, sc_lap_pe_dim))

                fc_lap_pe_dim = batch[0].fc_lap_pe.shape[-1]
                fc_lap_pos_enc = torch.zeros(
                    (len(batch), max_len_fc, fc_lap_pe_dim))

            sc_degree = None
            fc_degree = None
            use_degree = hasattr(
                batch[0], 'sc_degree') and batch[0].sc_degree is not None
            if use_degree:
            ...
            for i, g in enumerate(batch):
                ...
                g.sc_edge_attr = sc_edge_attr = torch.ones(
                    g.sc_edge_index.size(1), dtype=torch.float)
                sc_edge_indice.append(g.sc_edge_index)
                sc_adj = torch.sparse_coo_tensor(
                    g.sc_edge_index, sc_edge_attr, size_sc)
                sc_adj = sc_adj.to_dense()
                adjs_sc[i, :g_len_sc, :g_len_sc] = sc_adj

                g_len_fc = len(g.fc_x)
                size_fc = torch.Size([g_len_fc, g_len_fc])
                g.fc_edge_attr = fc_edge_attr = torch.ones(
                    g.fc_edge_index.size(1), dtype=torch.float)
                fc_edge_indice.append(g.fc_edge_index)
                fc_adj = torch.sparse_coo_tensor(
                    g.fc_edge_index, fc_edge_attr, size_fc)
                ...
                if use_sc_pe:
                    sc_pos_enc[i, :g.sc_pe.shape[-2],
                               :g.sc_pe.shape[-1]] = g.sc_pe
                if use_sc_pe:
                    fc_pos_enc[i, :g.fc_pe.shape[-2],
                               :g.fc_pe.shape[-1]] = g.fc_pe
                if use_lap_pe:
                    # print("g_len", g_len, "g.lap_pe.shape[-1]", g.lap_pe.shape[-1], "g.lap_pe:", g.lap_pe.shape)
                    sc_lap_pos_enc[i, :g_len_sc,
                                   :g.sc_lap_pe.shape[-1]] = g.sc_lap_pe
                    fc_lap_pos_enc[i, :g_len_fc,
                                   :g.fc_lap_pe.shape[-1]] = g.fc_lap_pe
