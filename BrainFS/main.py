...
from model_components.MBT import DAC_GNet
...
parser.add_argument('--num_initial_gnn_layers', type = int, default = 2, help='use for DAC_GNet')
parser.add_argument('--num_coupling_layers', type = int, default = 2)
parser.add_argument('--model_name', type=str, default='Alternately_Attention_Bottlenecks', 
                    choices=['Alternately_Attention_Bottlenecks', 'DAC_GNet', 'Attention_Bottlenecks', 'GNN_Transformer', 'SubGraphGNN_Transformer', 'GNN_Bottlnecks'], 
...
tsne_dir = os.path.join('tsne', args.dataset) # e.g., 'tsne/ZDXX'
ckpt_dir = os.path.join('ckpt', args.dataset) # e.g., 'ckpt/ZDXX'
result_dir = os.path.dirname(args.result_path)

os.makedirs(tsne_dir, exist_ok=True) # exist_ok=True prevents error if directory already exists
os.makedirs(ckpt_dir, exist_ok=True) # exist_ok=True prevents error if directory already exists
if result_dir: # Ensure result_dir is not empty (e.g., if path is just 'file.txt') <-- ADD THIS
...
if __name__ == '__main__':
    ...
    for random_seed in random_s:
        ...
        for i, (train_split, valid_split, test_split) in enumerate(zip(*cross_validate(args.repetitions, myDataset))):
            ...
            # model = DAC_GNet(args)
            if args.model_name == 'Alternately_Attention_Bottlenecks':
                model = Alternately_Attention_Bottlenecks(args)
            elif args.model_name == 'DAC_GNet':
                model = DAC_GNet(args)
            elif args.model_name == 'GNN_Transformer':
                model = GNN_Transformer(args)
            else:
                   raise ValueError(f"Model {args.model_name} not recognized. Please choose from 'Alternately_Attention_Bottlenecks', 'DAC_GNet', 'Attention_Bottlenecks', 'GNN_Transformer', 'SubGraphGNN_Transformer', or 'GNN_Bottlnecks'.")
            if args.use_cuda:
                model = model.cuda()
